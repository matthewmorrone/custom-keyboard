 ...
 public void onDraw(Canvas canvas) {
  ...
  for (Key key : keys) {
   ...
   if (key.popupCharacters != null && PreferenceManager.getDefaultSharedPreferences(kcontext).getBoolean("hints", true)) {
    canvas.drawText(String.valueOf(key.popupCharacters.charAt(0)), key.x + 25, key.y + 40, mPaint);
    if (key.popupCharacters.length() > 1) {
     canvas.drawText(String.valueOf(key.popupCharacters.charAt(1)), key.x + (key.width - 25), key.y + 40, mPaint);
    }
    if (key.popupCharacters.length() > 2) {
     canvas.drawText(String.valueOf(key.popupCharacters.charAt(2)), key.x + 25, key.y + (key.height - 25), mPaint);
    }
    if (key.popupCharacters.length() > 3) {
     canvas.drawText(String.valueOf(key.popupCharacters.charAt(3)), key.x + (key.width - 25), key.y + (key.height - 25), mPaint);
    }
   }
   }
